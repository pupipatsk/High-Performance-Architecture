...
def greedy_dominating_set(n: int, reach: Dict[int, Set[int]]) -> Set[int]:
    covered = set()
    power_plants = set()

    while len(covered) < n:
        best_city = max(
            (c for c in reach if c not in power_plants),
...
def improve_solution(n: int, reach: Dict[int, Set[int]], plants: Set[int]) -> Set[int]:
    improved = set(plants)
    for city in sorted(plants):  # Sorted for determinism
...
def solve(n: int, graph: Dict[int, Set[int]]) -> str:
    reach = compute_reach_sets(graph)
    initial = greedy_dominating_set(n, reach)
    improved = improve_solution(n, reach, initial)
    return power_plant_string(n, improved)
